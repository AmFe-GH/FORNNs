...
class sche_lr:
    ...
    def __call__(self):
        if self.epoch_now < self.begin_epoch:
            ...
            new_lr = self.begin_lr * (self.beta) ** (np.ceil(progress_rate * self.times_beta))
        if self.ifplot:
            print("lr:", new_lr)
        self.epoch_now += 1
        return float(new_lr)
...
def X_rightfunc_Lorenz_numpy(x):
    val = np.array(
        [
            [
                10 * (x[0, 1] - x[0, 0]),
                28 * x[0, 0] - x[0, 1] - x[0, 0] * x[0, 2],
                x[0, 0] * x[0, 1] - 8 / 3 * x[0, 2],
            ]
        ]
    )
    return val
...
def X_rightfunc_Chua_numpy(x):
    ...
    val = np.array(
        [
            [
                10 * (0.7 * (x[0, 1] - x[0, 0]) - g_function_of_Chua(x[0, 0])),
...
def load_config(config_file="Hyperparameters.yaml"):
...
def draw3d_train_test(real_x, pre_x, real_t, N_step, test_step, fig_path=None):
    ...
    for index in range(3):
        # First subplot
        axs[index].plot(real_t, real_x[:, index], linewidth=2.2, color="steelblue")
        axs[index].plot(
            real_t[:N_step], pre_x[:N_step, index], linewidth=2.2, color="firebrick", dashes=[4, 3]
        )
        axs[index].plot(
            real_t[-test_step:],
            pre_x[-test_step:, index],
            linewidth=2.2,
            color="peru",
            dashes=[4, 3],
        )
    ...
    plt.figure(2)
    plt.subplot(1, 3, 1)
    plt.plot(pre_x[:N_step, 0], pre_x[:N_step, 1], linewidth=1.0, color="firebrick", dashes=[6, 2])
    plt.plot(
        pre_x[-test_step:, 0], pre_x[-test_step:, 1], linewidth=1.0, color="peru", dashes=[6, 2]
    )
    plt.plot(real_x[:, 0], real_x[:, 1], linewidth=1.0, color="steelblue", alpha=alpha)
    plt.xlabel("x")
    plt.ylabel("y")
    plt.subplot(1, 3, 2)
    plt.plot(pre_x[:N_step, 0], pre_x[:N_step, 2], linewidth=1.0, color="firebrick", dashes=[6, 2])
    plt.plot(
        pre_x[-test_step:, 0], pre_x[-test_step:, 2], linewidth=1.0, color="peru", dashes=[6, 2]
    )
    plt.plot(real_x[:, 0], real_x[:, 2], linewidth=1.0, color="steelblue", alpha=alpha)
    plt.xlabel("x")
    plt.ylabel("z")
    plt.subplot(1, 3, 3)
    plt.plot(pre_x[:N_step, 1], pre_x[:N_step, 2], linewidth=1.0, color="firebrick", dashes=[6, 2])
    plt.plot(
        pre_x[-test_step:, 1], pre_x[-test_step:, 2], linewidth=1.0, color="peru", dashes=[6, 2]
    )
    plt.plot(real_x[:, 1], real_x[:, 2], linewidth=1.0, color="steelblue", alpha=alpha)
    plt.xlabel("y")
    plt.ylabel("z")
    ...
    ax.plot(real_x[:, 0], real_x[:, 1], real_x[:, 2], linewidth=0.6, alpha=0.1, color="steelblue")
    ax = plt.gca()
    ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    if fig_path:
...
def draw3d(real_x, pre_x, show=True, save_path=None):
    ...
    for index in range(3):
        # First subplot
        axs[index].plot(real_x[:, index], linewidth=2.2, color="steelblue")
        axs[index].plot(pre_x[:, index], linewidth=2.2, color="firebrick", dashes=[4, 3])
    ...
    plt.plot(real_x[:, 0], real_x[:, 1], linewidth=2, label="real", alpha=alpha)
    plt.xlabel("x")
    plt.ylabel("y")
    plt.subplot(1, 3, 2)
    plt.plot(pre_x[:, 0], pre_x[:, 2], linewidth=2, label="pre", alpha=alpha)
    plt.plot(real_x[:, 0], real_x[:, 2], linewidth=2, label="real", alpha=alpha)
    plt.xlabel("x")
    plt.ylabel("z")
    plt.subplot(1, 3, 3)
    plt.plot(pre_x[:, 1], pre_x[:, 2], linewidth=2, label="pre", alpha=alpha)
    plt.plot(real_x[:, 1], real_x[:, 2], linewidth=2, label="real", alpha=alpha)
    plt.xlabel("y")
    plt.ylabel("z")
    plt.tight_layout()
    if save_path:
    ...
    ax.plot(real_x[:, 0], real_x[:, 1], real_x[:, 2], label="3D real", alpha=alpha)
    ax.legend()
    if save_path:
...
class fols_Fun_tensor_with_inputs:
    def __init__(self, feature_size, N, device):
        self.backbone = torch.nn.Sequential(
            torch.nn.Linear(N, 50 * N),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(50 * N, 10 * N),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(10 * N, 1),
        )
